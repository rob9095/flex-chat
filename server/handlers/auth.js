...
const refreshUserToken = (config) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			let signature = (({ id, email, emailVerified }) => ({ id, email, emailVerified }))(user)
			if (create) {
...
exports.signup = async function(req, res, next) {
	try {
		...
		if(err.code === 11000) {
			console.log(err)
			err.message = 'Sorry, that email has been taken.'
		}
		return next({
			status:400,
