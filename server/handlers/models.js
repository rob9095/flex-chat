...
const buildPopulateArray = (popArray) => {
	try {
		return popArray.map(popConfig=> {
			//create a match with the query if it exists
			...
			let populate = Array.isArray(popConfig.populate) ? buildPopulateArray(popConfig.populate) : popConfig.populate
			return({
				...popConfig,
				match,
				populate
...
exports.queryModelData = async (req, res, next) => {
	try {
		...
		let populateArray = Array.isArray(req.body.populateArray) && req.body.populateArray.length > 0
			? buildPopulateArray(req.body.populateArray)
			: []
...
updateModelDocumentsRefs = (config) => {
	let { updates, refUpdates, model, refModel } = config
	return new Promise(async (resolve,reject) => {
		try {
			//find all the recently updated docs
			let foundDocs = updates.length > 0 ? await db[model].find({ $and: [{ $or: updates.map(u => ({ ...u })) }] }) : []
			...
			let docRefUpdates = refUpdates.map(doc=>({
				updateOne: {
					filter: { [doc.filterRef]: doc[doc.filterRef] },
					update: {
...
exports.upsertModelDocuments = async (req,res,next) => {
	try {
		let updates = req.body.data.map(doc=>{
			let filter = {}
			req.body.filterRefs.forEach(fr => filter = {...filter, [fr]: doc[fr] } )
			return ({
				updateOne: {
					filter,
					update: {
...
exports.getAllModelDocuments = async (req,res,next) => {
	try {
		let { limit = 10, documentRef = {}, regex, populateArray = [], model } = req.body		
		if (limit > 100) {
			return next({
				status: 404,
